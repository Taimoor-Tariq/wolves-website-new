...
router.get('/dashboard/:page.:section?', checkSession, async (req, res) => {
    ...
    Promise.all(prom).then(async () => {
        if (!new_perms.includes(req.params.page.toUpperCase())) res.redirect('/dashboard');
            ...
            switch (req.params.section) {
                case "players":
                    let data = await db.wolvesDB.getPlayers();
                    page_info = {
                        title: "Players",
                        info: data
                    }
            ...
            res.send(pug.renderFile(`${__dirname}/../views/admin/dashboard_${req.params.page.toLowerCase()}.pug`, {
                style: sass.renderSync({file: `${__dirname}/../sass/admin/dashboard.scss`}).css.toString(),
                msg: req.query.msg,
                perms: req.session.user,
